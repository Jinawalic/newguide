...
const ActionsPreview = () => {
    ...
    const actions = [
        { icon: ViewIcon, label: 'Inspect', color: 'hover:text-blue-600', tip: 'Audit Trail' },
        { icon: EditIcon, label: 'Modify', color: 'hover:text-amber-600', tip: 'Meta Patch' },
        { icon: ShareIcon, label: 'Relay', color: 'hover:text-indigo-600', tip: 'Public Link' },
        { icon: TrashIcon, label: 'Purge', color: 'hover:text-rose-600', tip: 'Hard Delete' }
    ];
    ...
    return (
        <div className="w-full flex flex-col items-center gap-10 relative z-[100]">
            <div className="w-full max-w-sm bg-white p-10 rounded-xl border border-slate-100 shadow-sm relative group overflow-visible">
                <div className="mb-10 text-center">
                    <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase leading-none mb-2">Record Pivot</h3>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">In-line controls</p>
                </div>

                <div className="space-y-4">
                    {rows.map(row => (
                        <div key={row.id} className="px-5 py-3 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between relative">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-400 font-bold text-[10px]">
                                    #{row.id}
                                </div>
                                <div className="leading-none">
                                    <div className="text-[11px] font-black text-slate-900 uppercase tracking-widest mb-1">{row.type}</div>
                                    <div className="text-[9px] font-mono text-slate-400">{row.hash}</div>
                                </div>
                            </div>
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${openId === row.id ? 'bg-slate-900 text-white' : 'bg-white text-slate-400 shadow-sm border border-slate-100 hover:border-slate-300'
                                        }`}
                                >
                                ...
                                {openId === row.id && (
                                    <div className="absolute right-0 mt-3 w-56 bg-white border border-slate-100 rounded-xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.2)] p-3 z-50 animate-in fade-in slide-in-from-top-4 duration-500">
                                        <div className="grid grid-cols-1 gap-1">
                                            {actions.map((act, i) => (
                                                <button
                                                    key={i}
                                                    onClick={() => setOpenId(null)}
                                                    className={`w-full flex items-center justify-between px-5 py-2 rounded-xl transition-all text-slate-500 ${act.color} group/item hover:bg-slate-50`}
                                                >
                                                    <div className="flex items-center gap-4">
                                                        <HugeiconsIcon icon={act.icon} size={18} className="text-slate-300 group-hover/item:text-inherit" />
                                                        <div className="text-left leading-none">
                                                            <div className="text-[10px] font-black uppercase tracking-widest mb-0.5">{act.label}</div>
                                                            <div className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{act.tip}</div>
                                                        </div>
                                                    </div>
                                                    <HugeiconsIcon icon={Tick} size={14} className="opacity-0 group-hover/item:opacity-100 text-slate-200" />
                                                </button>
                                            ))}
                                        </div>
                                    </div>
    ...
    return (
        <div className="max-w-[1000px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="border border-slate-200 rounded-2xl bg-white overflow-hidden shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)]">
                <div className="h-[600px] bg-[#f8fafc] relative flex items-center justify-center border-b border-slate-100 overflow-visible">
