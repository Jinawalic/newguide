...
const TagPreview = () => {
    ...
    return (
        <div className="w-full flex flex-col items-center gap-10 relative z-[100]">
            <div className="w-full max-w-md bg-white p-8 rounded-xl border border-slate-100 shadow-2xl relative">
                <div className="mb-6">
                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] mb-2">Category Tags</h4>
                    <p className="text-sm font-bold text-slate-900 tracking-tight">Project Metadata</p>
                </div>

                <div className="flex flex-wrap gap-3 p-2 bg-slate-50 border border-slate-100 rounded-xl focus-within:ring-4 focus-within:ring-blue-500/10 focus-within:border-blue-500/30 transition-all">
                    {tags.map((tag, i) => (
                        <div key={i} className="flex items-center gap-2 pl-4 pr-1.5 py-1.5 bg-white border border-slate-200 text-slate-900 rounded-2xl text-[11px] font-black uppercase tracking-tight shadow-sm hover:shadow-md transition-shadow group">
                            <span>{tag}</span>
                            <button
                                onClick={() => setTags(tags.filter((_, idx) => idx !== i))}
                                className="w-6 h-6 rounded-xl flex items-center justify-center hover:bg-rose-50 hover:text-rose-500 transition-colors"
                            >
                    ...
                    <div className="flex-1 flex items-center min-w-[120px] px-3">
                        <HugeiconsIcon icon={Plus} size={16} className="text-slate-300" />
                        <input
                            value={input}
                            onChange={(e) => setInput(e.target.value)}
                            onKeyDown={handleAdd}
                            placeholder="Type and enter..."
                            className="w-full bg-transparent outline-none text-[11px] font-bold text-slate-600 px-2 placeholder:text-slate-300 placeholder:uppercase"
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="flex gap-4">
                <div className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-xl text-[10px] font-black uppercase tracking-widest border border-indigo-100">Batch Tagging</div>
                <div className="px-4 py-2 bg-emerald-50 text-emerald-700 rounded-xl text-[10px] font-black uppercase tracking-widest border border-emerald-100">{tags.length} Active</div>
            </div>
        </div>
    );
};
