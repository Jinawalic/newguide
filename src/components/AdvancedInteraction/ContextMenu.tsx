    ...
    return (
        ...
        >
            <div className="text-center space-y-4">
                <div className="w-20 h-20 bg-white rounded-3xl shadow-xl flex items-center justify-center mx-auto border border-slate-100 group-hover:scale-110 transition-transform duration-500">
                ...
                <div>
                    <p className="text-sm font-black text-slate-800 tracking-tight uppercase">Right-click here</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">To trigger context menu</p>
                </div>
            </div>

            {show && (
                <div
                    className="absolute z-[110] w-56 bg-white border border-slate-100 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] py-2 animate-in zoom-in-95 duration-200"
                    style={{ top: position.y, left: position.x }}
                    ...
                    <div className="px-1.5 space-y-0.5">
                        {['Edit Item', 'Duplicate', 'Share Link', 'Archive'].map((item, idx) => (
                            <React.Fragment key={item}>
                                {idx === 3 && <div className="h-px bg-slate-100 my-1.5 mx-2" />}
                                <button className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-[12px] font-bold transition-all
                                    ${idx === 3 ? 'hover:bg-rose-50 text-rose-600' : 'hover:bg-emerald-50 text-slate-700 hover:text-emerald-800'}
                                `}>
                                    {item}
                                    <span className="text-[9px] text-slate-400 font-black opacity-0 group-hover:opacity-100">
                                        {['⌘E', '⌘D', '⌘S', 'DEL'][idx]}
                                    </span>
                                </button>
                            </React.Fragment>
    ...
    return (
        <div className="max-w-[1000px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="border border-slate-200 rounded-2xl bg-white overflow-hidden shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)]">
                <div className="h-[400px] bg-[#f8fafc] relative overflow-hidden border-b border-slate-100">
