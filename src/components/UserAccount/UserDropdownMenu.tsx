    ...
    return (
        <div className="w-full h-full flex items-center justify-center relative z-[100] mb-[90px]">
            <div className="relative">
                <button
                    onClick={() => setIsOpen(!isOpen)}
                    className="group flex items-center gap-4 p-2 pr-6 bg-white border border-slate-100 rounded-xl shadow-sm hover:-translate-y-1 active:scale-95 transition-all duration-300"
                >
                    <div className="w-8 h-8 rounded-[22px] bg-slate-900 flex items-center justify-center text-white rotate-3 group-hover:rotate-0 transition-transform">
                    ...
                    <div className="text-left">
                        <p className="text-[13px] font-black text-slate-900 uppercase tracking-tighter leading-none mb-1">Alex Rivier</p>
                        <p className="text-[10px] font-bold text-emerald-500 capitalize tracking-widest">Premium Member</p>
                    </div>
                    <svg className={`w-4 h-4 text-slate-400 transition-transform duration-500 ${isOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path d="M19 9l-7 7-7-7" /></svg>
                </button>

                {isOpen && (
                    <div className="absolute right-0 top-full w-72 bg-white border border-slate-100 rounded-xl shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
                        <div className="p-4 mb-2">
                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] mb-4">Management</p>
                            <div className="space-y-1">
                                {items.map((item, i) => (
                                    <button
                                        key={i}
                                        onClick={() => setIsOpen(false)}
                                        className="w-full flex items-center gap-4 px-4 py-2 rounded-[24px] hover:bg-slate-50 group transition-all"
                                    >
                                        <div className={`w-7 h-7 rounded-2xl bg-${item.color}-50 flex items-center justify-center text-${item.color}-600 group-hover:scale-110 transition-transform`}>
                                            <HugeiconsIcon icon={item.icon} size={20} />
                                        </div>
                                        <span className={`text-[13px] font-black tracking-tight ${item.color === 'rose' ? 'text-rose-500' : 'text-slate-600'}`}>{item.label}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
